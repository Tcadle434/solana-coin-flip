...
declare_id!("2gJ8CLYn4n1SkwHWivLqyePSGCXs8g8CKZj5nGTx8jbE");
...
pub mod switchboard_vrf_flip {
    ...
    #[access_control(ctx.accounts.validate(&ctx, &params))]
    pub fn house_fund(ctx: Context<HouseFund>, params: HouseFundParams) -> anchor_lang::Result<()> {
        HouseFund::actuate(&ctx, &params)
    }
    #[access_control(ctx.accounts.validate(&ctx, &params))]
...
security_txt! {
    name: "Solana coin flip! Heavily inspired by Switchboard VRF Flip",
    project_url: "https://www.itsthomas.dev/",
    contacts: "twitter:blockchief",
    policy: "https://docs.switchboard.xyz/security",
    preferred_languages: "en",
    source_code: "https://github.com/switchboard-xyz/vrf-flip",
    auditors: "None",
    acknowledgements: "
    This program is heavily inspired by the Switchboard VRF Flip repo, but has been modified to only focus on the coin flip game and a central SPL pool managed by the owner.
    The original repo can be found here: https://github.com/switchboard-xyz/vrf-flip
    "
}
